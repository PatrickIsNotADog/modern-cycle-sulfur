...
!dump J-Values
  subroutine patmo_dumpJValue(fname)
	...
	COS+S->CS2+O, CO+S2->CS2+O, SCSOH->CS2+OH, COS+HSO2->SCSOH+O2, S+SO->S2+O, &
	COS->CO+S, CS2->CS+S, CS2->CS2E, SO2->SO+O, SO3->SO2+O, &
	H2S->SH+H, SO->S+O"
    !loop on cells
    do i=1,cellsNumber
        write(22,*) i, &
		krate(i,3), krate(i,4), krate(i,5), krate(i,6), krate(i,7), krate(i,8), &
		krate(i,42), krate(i,43),krate(i,44), krate(i,45), krate(i,58), &
		krate(i,59), krate(i,60), krate(i,61), krate(i,62), krate(i,99), &
		krate(i,46), krate(i,49), krate(i,50), krate(i,51), krate(i,52), &
		krate(i,53), krate(i,54)
  ...
  subroutine patmo_dumpAllRates(fname)
	...
	(Reverse Reation)&
	CS+SO->CS2+O, COS+S->CS2+O, CO+S2->CS2+O, SCSOH->CS2+OH, COS+HSO2->SCSOH+O2, S+SO->S2+O, &
	(Photochemistry)&
	COS->CO+S, CS2->CS+S, CS2->CS2E, SO2->SO+O, SO3->SO2+O, &
	H2S->SH+H, SO->S+O, CS+O2->COS+O, CS+O3->COS+O2, CS+O->CO+S, &
    ...
    do i=1,cellsNumber
        ...
        krate(i,58)*nall(i,patmo_idx_CS)*nall(i,patmo_idx_SO), &
        krate(i,59)*nall(i,patmo_idx_COS)*nall(i,patmo_idx_S), &
        krate(i,60)*nall(i,patmo_idx_CO)*nall(i,patmo_idx_S2), &
        krate(i,61)*nall(i,patmo_idx_SCSOH), &
		krate(i,62)*nall(i,patmo_idx_COS)*nall(i,patmo_idx_HSO2), &
		krate(i,99)*nall(i,patmo_idx_S)*nall(i,patmo_idx_SO), &
		krate(i,46)*nall(i,patmo_idx_COS), &
		krate(i,49)*nall(i,patmo_idx_CS2), &
		krate(i,50)*nall(i,patmo_idx_CS2), &
		krate(i,51)*nall(i,patmo_idx_SO2), &
		krate(i,52)*nall(i,patmo_idx_SO3), &
		krate(i,53)*nall(i,patmo_idx_H2S), &
		krate(i,54)*nall(i,patmo_idx_SO), &
  ...
  subroutine patmo_dumpAllReverseRates(fname)
	...
	2O2+SH->HSO+O3, SO+O->S+O2, O2+SO->S+O3, H+SO->S+OH&
	"
    !loop on cells
    do i=1,cellsNumber
        write(22,*) i, &
		krate(i,57)*nall(i,patmo_idx_SH)*nall(i,patmo_idx_COS), &
        krate(i,59)*nall(i,patmo_idx_COS)*nall(i,patmo_idx_S), &
        krate(i,58)*nall(i,patmo_idx_CS)*nall(i,patmo_idx_SO), &
        krate(i,63)*nall(i,patmo_idx_COS)*nall(i,patmo_idx_O), &
        krate(i,64)*nall(i,patmo_idx_COS)*nall(i,patmo_idx_O2), &
        krate(i,60)*nall(i,patmo_idx_CO)*nall(i,patmo_idx_S2), &
        krate(i,99)*nall(i,patmo_idx_S)*nall(i,patmo_idx_SO), &
        krate(i,73)*nall(i,patmo_idx_SO2)*nall(i,patmo_idx_O2), &
        krate(i,74)*nall(i,patmo_idx_SO2)*nall(i,patmo_idx_O), &
        krate(i,75)*nall(i,patmo_idx_SO2)*nall(i,patmo_idx_H), &
        krate(i,61)*nall(i,patmo_idx_SCSOH), &
        krate(i,62)*nall(i,patmo_idx_COS)*nall(i,patmo_idx_HSO2), &
        krate(i,84)*nall(i,patmo_idx_HO2)*nall(i,patmo_idx_SO2), &
        krate(i,70)*nall(i,patmo_idx_H)*nall(i,patmo_idx_SO), &
		krate(i,71)*nall(i,patmo_idx_OH)*nall(i,patmo_idx_SO), &
		krate(i,72)*nall(i,patmo_idx_HSO)*nall(i,patmo_idx_O2), &
		krate(i,82)*nall(i,patmo_idx_SO2)*nall(i,patmo_idx_OH), &
		krate(i,83)*nall(i,patmo_idx_O2)*nall(i,patmo_idx_O2)*nall(i,patmo_idx_SH), &
		krate(i,77)*nall(i,patmo_idx_SO)*nall(i,patmo_idx_O), &
		krate(i,78)*nall(i,patmo_idx_O2)*nall(i,patmo_idx_SO), &
		krate(i,79)*nall(i,patmo_idx_S)*nall(i,patmo_idx_SO)
